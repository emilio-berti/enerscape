...
# Example 1 - calculating energy landscape of the Stromboli Island

In this example, we want to derive the energy landscape on the volcanic island of Stromboli (Sicily) and compute the least cost paths between two points laying on opposite sides of the volcano. We first download the digital elevation model (DEM) from TINITALY(http://tinitaly.pi.ingv.it).
...
tmp <- tempfile()
download.file("http://tinitaly.pi.ingv.it/data/w43010_s10/w43010_s10.zip", tmp)
...
dem[dem == 0] <- NA
m <- dem
m[!is.na(dem)] <- 1 #just a mask
plot(dem)
```

We use the `enerscape()` function to calculate the energy landscape.

```{r energy landscape}
en <- enerscape(dem, 10, unit = "kcal")
```
```{r explore en}
names(en)
```
 * `neighbors` is the number of neighbor cells to consider when building the transition matrix This can be 4 (chess rook's moves of 1, default), 8 (chess king's moves) or 16 (chess king's + knight's moves). You can find more help in the documentation of the *gdistance* package. 
  * `mass` is the body mass (kg) specified as input
  * `rasters` is a rasterstack with DEM, slope, work, and conductance
  * `cont_tr` is the transition matrix (from *gdistance*) that can be used for connectivity analyses.

As 16 neighbors may increase accuracy of the calculations, we re-ran calculations with `neigh = 16`.

```{r en 16}
en <- enerscape(dem, 20, unit = "kcal", neigh = 16)
```

We plot the output rasters.

```{r plot en}
s <- en$rasters * m
names(s) <- names(en$rasters)
plot(s) #mask NA values
```

We then calculate the least cost paths between two points A and B. We calculate both from A to B and from B to A, as energy costs of transport depend on inclines and the two paths can thus differ.
...
A <- p[1, ]
B <- p[2, ]
lcp_AB <- en_lcp(en, neigh = 16, or = A, dest = B)
lcp_BA <- en_lcp(en, neigh = 16, or = B, dest = A)
```
...
And plot them together with costs and distances.

```{r short plot}
plot(dem)
points(p, pch = 20, col = "steelblue")
text(1038500, 4311800, "A")
text(1041000, 4311800, "B")
lines(lcp_AB$`least-cost path`$Path, lw = 2, lt = 2)
text(1041000, 4313500, paste(round(lcp_AB$`least-cost path`$Cost), "kcal"))
text(1041000, 4313800, paste(round(lcp_AB$`least-cost path`$Distance / 1000, 2), "km"))
lines(lcp_BA$`least-cost path`$Path, lw = 2, lt = 4)
text(1040500, 4312500, paste(round(lcp_BA$`least-cost path`$Cost), "kcal"))
text(1040500, 4312800, paste(round(lcp_BA$`least-cost path`$Distance / 1000, 2), "km"))
```

Here, the least-cost path from A to B is comparable to the one from B to A, with minimal differences in trajectory, distance travelled, and total costs.
...
```{r cs results}
# cs <- raster("/home/eb97ziwi/Proj/enerscape-paper/circuitscape/cs_cum_curmap.asc")
# cs <- projectRaster(cs, dem)
# hist(log10(cs), breaks = 50)
# q <- quantile(log10(cs), c(0.1, 0.5, 0.9))
# cs_mask <- list()
# for (th in seq_along(q)) {
#   ans <- log10(cs)
#   ans[ans < q[th]] <- NA
#   ans[!is.na(ans)] <- 1
#   cs_mask[[th]] <- ans
# }
# names(cs_mask) <- names(q)
# cs_mask <- stack(cs_mask)
# plot(dem)
# plot(cs_mask$X90., add = TRUE, legend = FALSE,
