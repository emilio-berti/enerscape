...
A <- p[1, ]
B <- p[2, ]
lcp_AB <- en_lcp(en, or = A, dest = B)
lcp_BA <- en_lcp(en, or = B, dest = A)
```
...
text(0.4, 0.45, paste(round(lcp_AB$Cost), "kcal"))
text(0.4, 0.40, paste(round(lcp_AB$Distance, 2), "km"))
lines(lcp_BA$`least-cost path`$Path, lw = 2, lt = 4)
text(0.5, 0.2, paste(round(lcp_BA$Cost), "kcal"))
text(0.5, 0.15, paste(round(lcp_BA$Distance, 2), "km"))
```
