 ...
 * `neighbors` is the number of neighbor cells to consider when building the transition matrix This can be 4 (chess rook's moves of 1, default), 8 (chess king's moves) or 16 (chess king's + knight's moves). You can find more help in the documentation of the *gdistance* package. 
  ...
  * `cond_tr` is the transition matrix (from *gdistance*) that can be used for connectivity analyses.
...
text(0.4, 0.45, paste(round(lcp_AB$`least-cost path`$Cost), "kcal"))
text(0.4, 0.40, paste(round(lcp_AB$`least-cost path`$Distance, 2), "km"))
lines(lcp_BA$`least-cost path`$Path, lw = 2, lt = 4)
text(0.5, 0.2, paste(round(lcp_BA$`least-cost path`$Cost), "kcal"))
text(0.5, 0.15, paste(round(lcp_BA$`least-cost path`$Distance, 2), "km"))
```
